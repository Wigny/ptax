defmodule PTAX.Conversor do
  ...
  @type opts :: %{
          ...
          data: Date.t(),
          operacao: operacao,
          tipo_boletim: Quotation.Boletim.t()
        }

  defguardp valid_operation?(operacao) when operacao in ~w[buy sell]a
  ...
  ## Exemplo
      ...
      iex> PTAX.Conversor.run(15, %{de: :BRL, para: :GBP, data: ~D[2021-12-24], operacao: :sell, tipo_boletim: PTAX.Quotation.Bulletin.Closing})
      {:ok, #Decimal<1.9772>}
      iex> PTAX.Conversor.run(5, %{de: :USD, para: :BRL, data: ~D[2021-12-24], operacao: :buy, tipo_boletim: PTAX.Quotation.Bulletin.Closing})
      {:ok, #Decimal<28.2705>}
  """
  ...
  def run(valor, opts) when valid_params?(opts) and has_base?(opts) do
    ...
    with {:ok, %{^operacao => taxa}} <- Quotation.get(moeda_cotada, data, tipo_boletim) do
