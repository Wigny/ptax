defmodule PTAX.Conversor do
  ...
  ## Exemplo
      ...
      {:ok, #Decimal<28.2705>}
  """
  @spec run(valor :: PTAX.valor(), opts :: opts) :: {:ok, PTAX.valor()} | {:error, Error.t()}

  def run(valor, opts) when valid_params?(opts) and has_base?(opts) do
    %{de: de, para: para, data: data, operacao: operacao, boletim: boletim} = opts
    {moeda_cotada, base_conversor} = cotar([de, para])

    with {:ok, %{^operacao => taxa}} <- Cotacao.get(moeda_cotada, data, boletim) do
